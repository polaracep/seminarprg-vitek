...
namespace SortingPlayground
{
    internal class Program
    {
        ...
        static int[] SelectionSort(int[] array)
        {
            ...
            max = sortedArray.Max(x => x);
            for(int n =  1; n < sortedArray.Length;n++)
            {
                int max2 = sortedArray[0];
                for (i = 1; i < sortedArray.Length; i++)
                {
                    if (sortedArray[i] > max2)
                    {
                        max = sortedArray[i];

                    }
                    sortedArray[i] = sortedArray[sortedArray.Length - n];
                    sortedArray[sortedArray.Length - n] = max;
                }
